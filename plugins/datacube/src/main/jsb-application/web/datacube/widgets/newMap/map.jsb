{
    ...
    $client: {
        ...
        $constructor: function(opts){
            ...
            JSB.loadScript(['datacube/widgets/newMap/leaflet/leaflet.js', 'datacube/widgets/newMap/fuzzy.min.js'], function(){
                $this.setInitialized();
            });
        ...
        refresh: function(opts){
            ...
            var mapsContext = this.getContext().find('geojsonMaps').values(),
                newMapHash = [],
                maps = [];

            for(var i = 0; i < mapsContext.length; i++){
                var desc = mapsContext[i].find('geojson').value();

                newMapHash.push(desc);
                ...
                maps.push({
                    data: null,
                    desc: desc,
                    name: mapsContext[i].find('name').value(),
                    regionName: mapsContext[i].find('regionName').value(),
                    wrapLongitude: mapsContext[i].find('wrapLongitude').value()
                });
            }

            newMapHash = this.createFilterHash(newMapHash);
                ...
                try{
                    ...
                    while(dataSource.next()){
                        for(var i = 0; i < regionsContext.length; i++){
                            ...
                            if(!regions[i]){
                                regions[i] = {
                                    linkTo: regionsContext[i].find('linkTo').value(),
                                    data: []
                                };
                            }
        ...
        innerBuildChart: function(data){
            ...
            try {
                ...
                for(var i = 0; i < tileMaps.length; i++){
                    var url = tileMaps[i].find('url').value();
                    if(url){
                ...
                for(var i = 0; i < this._maps.length; i++){
                    if(this._maps[i].data){
                        ...
                        for(var j in data.regions){
                            if(data.regions[j].linkTo === this._maps[i].name){
                                linkedData = data.regions[j];
                                break;
                            }
                        }
        ...
        findRegion: function(region, array){
            ...
            for(var j = 0; j < array.length; j++){
                ...
                var sc = fuzzy(name, region).score;
                if(sc > score){
                    reg = array[j];
                    score = sc;
                }
    ...
    $server: {
        loadMaps: function(workspace, maps){
            for(var i = 0; i < maps.length; i++){
                ...
                maps[i].data = JSON.stringify(workspace.readArtifactAsJson(maps[i].desc.id + '.data'));
            }
