{
    $name: 'DataCube.Controls.JsonBindingSelector',
	...
	$client: {
	    ...
	    $constructor: function(opts){
	        $base(opts);
	        this.addClass('jsonBindingSelector');
            this.loadCss('JsonBindingSelector.css');

            this.placeholderElt = this.$('<div class="placeholder">Перетащите json-файл</div>');
                ...
                accept: function(d){
                    if(d && d.length > 0 && d.get(0).draggingItems){
                        for(var i in d.get(0).draggingItems){
                            ...
                            if(JSB.isInstanceOf(entry,'DataCube.Model.JsonFile')){
                                return true;
                            }
                        }
                    }
