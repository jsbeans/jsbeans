...
## Встраивание SQL выражений

`{$sql: 'sql expression'} - позволяет встроить SQL выражение в виде текста.

Для использования полей куба внутри SQL выражения - `$cube."имя поля"`, иначе необходимо указывать полное имя поля таблицы.
