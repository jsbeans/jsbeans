...
Статья о том, как мы собирали ""Кубик-Рубика"" - конструктор конструкторов.
...
_Рис 3. Структура срезов куба "Моссковские школы"_
...
_Рис 4. Срез "Оллимпиады по возрастам и годам"_
...
<<Запрос заменить, он не оптимальный>>
...
В результате такой комбинации для одного входного запроса мы получаем несколько вариантов его исполнения, как с использованием разных источников, так и с разными стратегиями выполнения - выбор основной/целевой базы данных, в которой будет выполняться главная часть запроса, сборка результатов.
...
Для более сложных случаем в системе реализован внутренний in-memory  интерпретатор запросов на базе H2 Embedded, который позволяет "из коробки" интегрировать между собой любые поддерживаемые базы данных. Буквально можно представить, что запрос разбивается на куски по группам источников, отправляется на выполнение, после чего производится сборка и финальная обработка результатов в памяти.

Такая схема интеграции данных на уровне внутреннего интерпретатора на первый взгляд кажется "тяжелой", и это правда, если приходится работать с большими объемами входных данных и необходимостью производить вычисления уже после пересечений и объединений наборов из внешних источников. В действительности это обстоятельство частично нивелируется - одновременно запрос выполняется несколькими обработчиками, следовательно интерпретатор используется только в самых крайних случаях. Например, когда одновременно источники не поддерживают запрос целиком, его не удается разбить на части, а целевая БД не поддерживает внешние источники. В итоге интеграция ограничена типичными транспортными издержками на подготовку, передачу по сети и прием данных, а это уже совсем другая задача и лежит скорее в плоскости организационных процедур.
...
Кубик рубика целиком и полностью реализован на JavaScript в виде множества js-бинов (*.jsb файлов), часть из которых функционируют только на сервере, другая часть на клиенте, а остальные представляют собой составной компонент, функционирующий как распределенное целое, части которого сами собой функционирую, прозрачно для разработчика, но под его контролем. Бины могут иметь разную жизненную стратегию на все случаи жизни, например, с привязкой или без к сесии пользвоателя. Бин является изоморфным, позволяет и на клиенте, и на сервере работать как с виртуальным экземпляром обычного класса. Бин описывается одним файлом и включает три секции - для методов, выполяющихся на клиенте, для серверных методов, и секция общих синхронизируемых полей.
...
По-началу, первичных наборов данных было мало. Предметные области и задачи были вполне конкретизированы. Казалось бы, зачем такие мучения? Всем хотелось получить результат сразу - особенно, когда быстрое решение лежало на поверхости, а правильное требовало усидчивости и взвешенных решений, соблюдения изначальной установки. Мы шли от сложных и долгих решений к простым и быстрым, по пути обобщения частных задач.
...
_В зависимости от реакции и предпочтений читателя, и решения модератора, в следующих статьях мы с радостью раскроем другие стороны некоторых интересных архитектурных решений нашей команды - конструируемую подсистему визуализации, аналитические процессы, язык запросов, импорт данных и что-то еще..._
