		...
		reloadScheme: function(){
		    ...
		    type=object
		        ...
		        - работает в двух режимах:
		            -- scheme - описывает структуру элементв (текущего дочернего JSON объекта) и имеет identity, который содержит имя свойства в scheme, которое позволяет привязать дескриптор схемы у узлу запроса;
		            -- key/value - описывает тип строкового ключа/свойства и значения, позволяет задавать именованные значения
            ...
            Специальные объекты:
                - #query - главный дескриптор запроса
                - #outputFieldName - имя выходного поля текущего запроса
