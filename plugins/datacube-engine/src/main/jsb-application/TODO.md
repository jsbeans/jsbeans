...
- поддержать отображение перечня связанных срезов при отображении полей/измерений в редакторе куба
- поддержать работу со срезами-заменителями (автоматически предварительно устанавливать связи и гуй для отображения и отключения)
- поддержать работу с кубом источником (предлагать только существующие поля, с учетом срезов и уже использованных)  
- в редакторе срезов поддержать задание типа при выборе поля ($type - наш внутренний тип, $nativeType - тип оригинальной БД)
- в редакторе срезов - отображать коллизии типов, как минимум в $join.$filter (если сравниваются несовместимые типы)
- поддержать приведение типов полей и констант (опция $type)
...
- TransformInterpreter - базовая подготовка запроса (конфигурируется)
- SQLTranslatorEngine <- SQLDefaultEngine, ClickHousEngine, PostgresEngine
- MongoTranslatorInterpreter
...
- переделать построение запроса, чтобы они были как $from запрос к срезу, как не запрос от запроса slice.getQuery()
