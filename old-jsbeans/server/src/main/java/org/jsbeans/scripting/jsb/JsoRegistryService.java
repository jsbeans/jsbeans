...
import org.jsbeans.scripting.JsHubService;
...
//        this.self().tell(JsoRegistryMessage.LOAD_JSO, this.getSelf());
        ...
        String resourcePath = ConfigHelper.getConfigString("kernel.jso.lookupResourcesPath");
...
*/
    ...
    private void loadObjects(String folder, boolean isServer) throws PlatformException {
        ...
        if (!isServer) {
            ...
            Collection<Entry<String, String>> set = ReflectionHelper.getPlatformReflections().getStore().get(ResourcesScanner.class).entries();
            for (Entry<String, String> e : set) {
                if (e.getValue().startsWith(folder) && (e.getValue().endsWith(".jso") || e.getValue().endsWith(".jsb") || e.getValue().endsWith(".jss"))) {
                    jsoPaths.add("/" + e.getValue());
                }
            }
        }
        for (String jsoPath : jsoPaths) {
            ...
            try {
                ...
                String codeToExecute = String.format("JSO().wrapJSO('%s','%s',function(){ %s })", jsoFile, relPath, JsoTemplateEngine.perform(jsoBody, jsoFile));
                ...
                Future<Object> f = ActorHelper.futureAsk(ActorHelper.getActorSelection(JsHubService.class), scriptMsg, ActorHelper.getServiceCommTimeout());
                ...
                if (result instanceof UpdateStatusMessage) {
                    ...
                    if (msg.error != null && msg.error.trim().length() > 0) {
                        getLog().error(String.format("Failed to perform JSO descriptor '%s' due to following reson: %s", jsoFile, msg.error));
                    } else {
                ...
                getLog().error(String.format("Failed to perform JSO descriptor '%s' due to following reson: %s", jsoFile, e.getMessage()));
            }
        }
    }
    ...
    private void loadJSO() throws PlatformException {
        try {
            String jsoPath = ConfigHelper.getConfigString("kernel.jso.jsoEngineResource");
            ...
            Future<Object> f = ActorHelper.futureAsk(ActorHelper.getActorSelection(JsHubService.class), execMsg, ActorHelper.getServiceCommTimeout());
            Object result = Await.result(f, ActorHelper.getServiceCommTimeout().duration());
            if (result instanceof UpdateStatusMessage) {
                UpdateStatusMessage msg = (UpdateStatusMessage) result;
                if (msg.error != null && msg.error.trim().length() > 0) {
        ...
        this.createChain(String.class, UpdateStatusMessage.class, jsoName)
                .add(new FutureMonad<String, Object>(msg) {
                    ...
                    public Future<Object> run(String name) throws PlatformException {
                        // try to lookup locally via JsHubService
                        ...
                        return ActorHelper.futureAsk(
                                ActorHelper.getActorSelection(JsHubService.class),
                                execMsg,
                    ...
                    this.createChain(JsonObject.class, UpdateStatusMessage.class, obj)
                            .add(new FutureMonad<JsonObject, Object>(sessionId, clientAddr, user, clientRequestId, userToken) {
                                ...
                                public Future<Object> run(JsonObject obj) throws PlatformException {
                                    ...
                                    return ActorHelper.futureAsk(ActorHelper.getActorSelection(JsHubService.class), execMsg, ActorHelper.getServiceCommTimeout());
                                }
