...
*/
    ...
    private String setTimeoutOrInterval(boolean isTimeout, ScriptableObject callback, long duration) {
        ActorSelection jsHubSvc = ActorHelper.getActorSelection(JsHubService.class);
...
/*		
		synchronized(mutex){
			askCounter++;
			final Logger L = LoggerFactory.getLogger(JsBridge.class.getName());
			L.debug("Ask count: " + askCounter);
		}
*/
        ActorSelection jsHubSvc = ActorHelper.getActorSelection(JsHubService.class);
        Context ctx = Context.enter();
