...
import org.jsbeans.servicemanager.BaseService;
...
public class JsHubService extends BaseService {
    private final long gcInterval = conf("gcInterval", 5 * 60 * 1000);
    private final boolean openDebugger = conf("openDebugger", false);
    private final long completeDelta = conf("stateCompleteTimeout", 10 * 60 * 1000);
    private final long execDelta = conf("stateExecutingTimeout", 1 * 60 * 60 * 1000);
    private final long sessionExpire = conf("sessionExpire", 1 * 60 * 60 * 1000);
    private final int jsOptimizationLevel = conf("jsOptimizationLevel", -1);
    private final int jsLanguageVersion = conf("jsLanguageVersion", Context.VERSION_1_8);
    ...
    public void start() {
        ...
        tickScheduler = this.serviceManager.schedule(
                () -> serviceManager.tell(JsHubService.class, new Tick()),
                gcInterval,
                false);
    }
    ...
    public void stop() {
        try {
            tickScheduler.cancel(true);
        } finally {
            super.stop();
        }
    }

    private void openDebugger(DebuggerMessage msg) {
        if (!msg.isStop()) {
            if (debugger == null) {
    ...
    public CompletableFuture<?> apply(Command<?> cmd) {
        ...
        if (cmd instanceof ExecuteScriptMessage) {
            return this.executeScript((ExecuteScriptMessage) cmd);
        } else if (cmd instanceof QueryStatusMessage) {
            return this.proceedQueryExecutionStatus((QueryStatusMessage) cmd);
        } else if (cmd instanceof JsAskMessage) {
            return this.handleJsAsk((JsAskMessage) cmd);
        } else if (cmd instanceof JsTimeoutMessage) {
            return this.handleJsTimeout((JsTimeoutMessage) cmd);
        } else if (cmd instanceof JsDumpMessage) {
            return this.handleJsDumpMessage((JsDumpMessage) cmd);
        } else {
            return this.utilizeMessage(cmd);
        }
    }
    ...
    private CompletableFuture<?> utilizeMessage(Message msg) {
        if (msg == null) {
            L.warn("Utilized null message ");
        } else {
            L.warn("Utilized unused message type {}", msg.getClass().getName());
        }
        return CompletableFuture.completedFuture(new NullPointerException("Message is null"));
    }
...
*/
                        ...
                        if (msg.isTell()) {
                            targetActor.tell(msgToSend, JsHubService.this.getSelf());
...
//	}
    ...
    private void updateSessions() {
        try {
            ...
            getLog().error(e, "JsHubService failed to collect garbage sessions with message: " + e.getMessage());
        }
    }
...
*/
    ...
    private static class Tick implements Message {
