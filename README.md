 ...
 * Расширения бинов (traits), позволяющие навесить на бины дополнительный функционал (AOP).
...
## Что такое бины?
В соответствии с концепцией jsBeans вся логика приложений (как серверная, так и клиентская) строится в виде иерархий объектов на языке JavaScript, напоминающих по своей сути EJB-компоненты в Java EE. Они изоморфны, соответствуют основным принципам ООП, могут порождаться и удаляться, могут вызываться удаленно и мигрировать с одного сервера на другой или в браузер. Такие объекты мы называем - **бинами**.
...
Таким образом вышеприведенный бин может быть одинаково использован как стороне сервера, так и на стороне клиента.
```javascript
...
Поля и методы, объявленные в общей секции бина (вне ```$server``` и ```$client```) являются общими и для сервера и для клиента и могут использоваться в коде обоих секций. JsBeans позволяет синхронизировать общие поля таким образом, что поле измененное кодом на одной стороне автоматически обновится и на другой. 
